        ...
        if (files.length > 0) {
            ...
            for (let id in user_token) {
                try {
                    ...
                    const save = await this.FileReader(path.join(savePath, user_token[id], 'save.json'))
                    const rks = Number(save?.saveInfo?.summary?.rankingScore)
                    if (!save) continue
                    if (!Number.isFinite(rks)) {
                        logger.mark('[phi-plugin][数据转移，请勿中断进程]', `奇怪的rks ${save?.saveInfo?.summary?.rankingScore}`)
                        continue
                    }
                    await getRksRank.addUserRks(user_token[id], rks)
                } catch (err) {
