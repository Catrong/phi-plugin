    ...
    async bind(e) {
        ...
        if (sessionToken == "qrcode") {
            ...
            if (Config.getUserCfg('config', 'TapTapLoginQRcode')) {
                qrCodeMsg = await send.send_with_At(e, [`请扫描二维码进行登录！如只有一个设备请直接长按识别二维码登录嗷！请勿错扫他人二维码。请注意，登录TapTap可能造成账号及财产损失，请在信任Bot来源的情况下扫码登录。`, segment.image(await getQRcode.getQRcode(request.data.qrcode_url))], false, { recallMsg: 60 });
            } else {
