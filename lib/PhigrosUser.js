...
class PhigrosUser {
     ...
     */
    async getSaveInfo() {
        let saveInfo = this.global ? await SaveManagerGB.saveCheck(this.session) : await SaveManager.saveCheck(this.session);

        let flag = false;
        saveInfo = saveInfo.filter(e => e.user.objectId != '6a265effd774134774ac90d6')
        // console.info(this.saveInfo)

        if (saveInfo[0] && saveInfo[0].createdAt) {
            /**多个存档默认选择第一个 */
            // this.saveInfo = this.saveInfo[0]
            this.saveInfo = saveInfo[0]
        } else {
