...
class PhigrosUser {
    ...
    async buildRecord() {
        ...
        if (saveUrl) {
            ...
            /**获取 gameProgress */
            let file = new ByteReader(/** @type {Buffer} */ (await savezip.file('gameProgress')?.async('nodebuffer')))
            file.getByte()
            this.gameProgress = new GameProgress(await SaveManager.decrypt(file.getAllByte()))

            /**获取 gameuser */
            file = new ByteReader(/** @type {Buffer} */ (await savezip.file('user')?.async('nodebuffer')))
            file.getByte()
            this.gameuser = new GameUser(await SaveManager.decrypt(file.getAllByte()))

            /**获取 gamesetting */
            file = new ByteReader(/** @type {Buffer} */ (await savezip.file('settings')?.async('nodebuffer')))
            file.getByte()
            this.gamesettings = new GameSettings(await SaveManager.decrypt(file.getAllByte()))

            /**获取gameRecord */
            file = new ByteReader(/** @type {Buffer} */ (await savezip.file('gameRecord')?.async('nodebuffer')))
