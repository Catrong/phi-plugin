    ...
    static async saveCheck(session) {
        ...
        if (size == 0)
            ...
            for (var i in array) {
                ...
                var date = new Date(array[i].updatedAt)
                array[i].updatedAt = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()} ${date.getHours()}:${date.getMinutes()}`
            }
        }
